...
Version 2.0 (Combined Edition) · October 2025
...
- **Applying to college** and they ask about strengths → Can't export your results, manual copy-paste
- **Job search platform** wants to match you by strengths → No API to connect, data is locked
- **Therapist** wants to see your profile → Send screenshots or pay again for a shareable report
- **LinkedIn profile** could showcase validated strengths → Can't integrate, platform won't allow it
...
> "I habitually engage in metacognitive reflection regarding my behavioral patterns."

**Same thing, said clearly:**
> "I often think about why I do the things I do."

Why make it complicated? Because most tests are written by researchers for other researchers, not for the diverse populations who need these tools.
...
Think of a flower with six petals. Each petal represents a different type of strength that research shows is distinct and important:
...
**Why not five like Big Five personality?** Five domains are too broad for actionable insight. You need more specificity.

**Why not 24 like VIA Character Strengths or 34 like CliftonStrengths?** Too many categories with too much overlap. VIA's "Curiosity" and "Love of Learning" correlate at r > .60—they're basically measuring the same thing. CliftonStrengths has similar redundancy.
...
**Our approach: Anchor-Based Generation with Multi-Layer Verification**

**1. Every AI question links to a validated "anchor"**

We start with 3,814 proven questions from research (IPIP, validated personality instruments). AI doesn't create questions from scratch—it creates variations of known-good items.

**2. Natural Language Inference checks semantic equivalence**

An NLI model (separate AI trained specifically for this) checks: "Does the AI question mean the same thing as the anchor?"

If the answer is "yes" with high confidence (entailment > 0.85), we keep it. If not, we reject it.

**3. Complete audit trail**

Every question has a paper trail:
- Original anchor question
- AI generation prompt
- NLI verification scores
- Final decision (keep/reject)
- User response (when someone answers it)

If a question performs poorly later, we can trace back exactly where it came from and what happened.

**4. Pilot validation before launch**

Before anyone takes the public assessment, we're running a pilot study with 1,000+ people to verify:

- Do AI-generated questions produce the same results as human-written anchors?
- Are reliability estimates (test-retest, internal consistency) equivalent?
...
> **🔬 For Researchers & Technical Evaluators**
>
> **The STEM Adapter Pipeline (Technical Specification)**
>
> **Phase 1: Generation**
>
> - **Model:** GPT-4o-mini (cost-effective, sufficient for item generation)
> - **Input:** Facet definition, anchor examples (2-3 items), user profile flags
> - **Constraints:** CEFR reading level (A2/B1/B2 based on user), word count (6-18), valence (positive/reverse), trauma filters
> - **Output:** 10-15 candidate items per facet, each explicitly linked to source anchor
...
> - **Logic:**
>   - **Positive-valence items:** `entailment - max(contradiction, neutral) ≥ 0.20` → KEEP
>   - **Reverse-valence items:** `contradiction - max(entailment, neutral) ≥ 0.20` → KEEP
>   - **Unclear:** REWRITE or DROP
> - **Backward retry:** If KEEP rate < 70% target, regenerate with adjusted prompts (max 3 attempts)
> - **Additional filters:** Word count validation, CEFR readability verification, diversity check (cosine similarity < 0.90)
>
> **Phase 3: Selection (In Development)**
>
> - Embed all KEEP items (text-embedding-3-small, 1536 dimensions)
> - Compute pairwise cosine similarity within facet
> - Rank by composite score: `NLI_confidence × anchor_quality × diversity_score`
> - Select top N items per facet (typically 5-8)
> - Exclude if similarity ≥ 0.90 to already-selected items
>
> **Provenance Tracking:**
>
...
> ```json
> {
>   "item_id": "gen_47392_curiosity_03",
>   "text": "I regularly research emerging trends in my field",
>   "facet": "Curiosity",
>   "anchor_id": "anchor_2847",
>   "generation": {
>     "model": "gpt-4o-mini",
>     "prompt_version": "v2.1.3",
>     "user_profile_flags": ["work_context", "trauma_sensitive"]
>   },
>   "nli_verification": {
>     "entailment": 0.89,
>     "contradiction": 0.04,
...
>
> 1. **IRT Parameter Inheritance:** Can AI-generated items inherit anchor IRT parameters based on NLI alignment (entailment > 0.85), or must we empirically recalibrate each item?
> 2. **Minimum Pilot Sample:** What N is required before deploying AI-enhanced items, even for low-stakes personal development use?
> 3. **Construct Drift Detection:** How do we monitor for semantic drift over time as item pools update?
> 4. **DIF Analysis:** What DIF thresholds are appropriate for AI-generated content across demographic groups?
>
> See Section 5 for complete pilot study design and validation protocol.
...
## Where We Are Now: Current Status & Timeline
...
**2. Anchor Item Database**

The foundation of validated questions is complete:

- **3,814 anchor items** from validated research sources (IPIP, peer-reviewed instruments)
- **IRT parameters** (discrimination, thresholds) from original studies where available
- **3,786 embeddings** (mathematical representations) for semantic matching
- **76 facet embeddings** for intelligent item-to-facet assignment
- Complete version control with audit trails
- Admin interface for managing items, bulk imports, quality checks

**What this enables:** We start with proven questions. We're not making things up—every AI-generated item links back to a validated anchor that's been tested on thousands of people.

**Why this matters for validation:** Having anchor items with known psychometric properties means we can compare AI-generated items to gold standards. If AI items perform differently than anchors, we'll catch it.

**3. STEM Adapter - Generation & Verification (75% Complete)**

Two of three phases are operational:

**Phase 1 - Generation (✅ Working):**
- AI creates 10-15 personalized questions per facet based on your profile
...
- Each generated item explicitly linked to source anchor

**Phase 2 - NLI Verification (✅ Working):**
- Natural Language Inference checks that generated questions mean the same thing as anchors
- Automatic retry if quality is insufficient (up to 3 attempts)
- Additional filters: reading level, word count, diversity (no redundant questions), trauma screening
- Complete audit trail of generation attempts and verification scores

**Phase 3 - Selection (🔄 In Development - Q1 2025):**
- Will rank all verified questions by quality and select the best 5-8 per facet
- Algorithms designed, implementation underway
- Expected completion: Q1 2025

**Current capability:** We can generate and verify personalized questions. What's missing is the final selection step to pick the optimal subset. This doesn't block pilot testing—we can use all verified items in pilot phase, then add selection optimization later.

---

### What's Not Started (Awaiting Expert Validation)
...
- Can AI-generated items inherit IRT parameters from anchors, or must we recalibrate empirically?
...
**Timeline:** Q2 2025, after expert consultation is complete.
...
**Timeline:** Q3 2026, after pilot study validates the core assessment.
...
**Timeline:** Q4 2026+, if everything before it succeeds.

---

### The Timeline: Phased Development with Validation Gates

We're not launching until we have evidence it works. Each phase has clear go/no-go decision points.

**Phase 0: Expert Consultation (Current - Q1 2025)**
...
**Deliverable:** Revised validation protocol co-authored with experts, green light to proceed with implementation

**Funding need:** $25K-50K for expert consultation and protocol design
...
**Phase 1: Complete Implementation (Q2 2025)**

**What we're building:**
- Finish STEM Adapter Selection phase (Phase 3)
...
**Decision gate:** Internal testing shows system works as expected, no major technical failures

**Deliverable:** Functioning assessment system ready for external pilot

**Funding need:** $20K-30K for development completion
...
**What we're testing:**
- Recruit 1,000-2,000 diverse participants
- Half take AI-personalized version, half take static anchor-only version (randomly assigned)
- All complete test-retest (take it twice, 2-4 weeks apart)
- Subset (N = 200) also take NEO-PI-R and VIA for convergent validity
...
**AI Performance:**
- Do AI-generated items perform equivalently to human-written anchors?
- Same reliability? Same factor loadings? Same freedom from bias?
...
- ✅ Reliability thresholds met (α/ω ≥ .70, test-retest r ≥ .80)
- ✅ Factor structure confirmed (adequate CFA fit indices)
- ✅ No unresolvable bias (can remediate problematic items)
- ✅ AI items perform comparably to anchors (if not, fall back to static item banks)

**If validation fails:** We revise and re-test, or acknowledge the approach doesn't work and pivot to traditional methods.

**Deliverable:** Pilot data, analysis results, decision on whether to proceed to public launch

**Funding need:** $75K-100K (participant recruitment and compensation, data collection infrastructure, statistical analysis)

---

**Phase 3: Validation Studies & Peer Review (Q1-Q2 2026, if pilot successful)**

**What we're doing:**
- Independent sample (N ≥ 2,000) for factor structure confirmation
- Longitudinal follow-up (6-12 months) for predictive validity
- Cross-cultural validation (translation to Spanish, Mandarin with separate validation)
- Write up results for peer-reviewed publication
- Release de-identified dataset publicly (with participant consent)

**Why this matters:** Peer review is how science works. Independent experts will critique our methods, challenge our conclusions, and verify we didn't cherry-pick results. This builds credibility.

**Decision gate:** Peer-reviewed publication accepted, community feedback is constructive (not "this is fundamentally flawed")

**Deliverable:** Published validation study, public dataset, scientific credibility

**Funding need:** $50K-75K (larger sample, longitudinal tracking, publication fees, dataset preparation)

---

**Phase 4: Public Beta Launch (Q3 2026, if validation successful)**
...
**Funding need:** $25K-50K (beta platform, API infrastructure, developer documentation)

---

**Phase 5: Integration Marketplace & Scale (Q4 2026+)**
...
- Item pool updates (add new anchors, retire poor performers)
...
**Risk 1: AI-Generated Items Don't Validate**

**What could happen:** Pilot study shows AI-generated items have lower reliability, weaker factor loadings, or introduce bias compared to human-written anchors.

**Why this could happen:** High NLI alignment (semantic similarity) doesn't guarantee psychometric equivalence. Subtle wording changes could alter what's being measured.

**Mitigation:**
- Pilot study is designed to catch this (AI vs. anchor comparison)
- If AI items underperform, we fall back to traditional fixed item banks
...
**What could happen:** 36 facets × 5-8 items per facet = 180-290 items total. Stable IRT parameters require large samples. If our pilot (N = 1,000-2,000) is insufficient, parameter estimates will be unstable.

**Why this could happen:** Rule of thumb is 200+ responses per item for stable GRM parameters. We have 1,000-2,000 people responding to 180-290 items. Math depends on item pool structure (are all items seen by all people, or only subset via adaptive sampling?).

**Mitigation:**
- Expert consultation (Phase 0) will determine minimum N before we start recruiting
- May need larger pilot sample (N = 2,000-5,000) if experts recommend
- Could use Bayesian IRT with informative priors from IPIP anchor data
...
**Impact on funding:** Larger sample = higher recruitment costs ($100K-150K instead of $75K-100K)
...
> **🔬 For Researchers & Technical Evaluators**
>
> **Complete Validation Protocol (Phase 2 - Pilot Study)**
>
> **Sample Design:**
> - **N ≥ 1,000** (target 1,500 for safety margin)
...
>
> **Design:**
> - **Between-subjects:** Random assignment to AI-personalized vs. static anchor-only version
> - **Within-subjects:** All participants complete test-retest (2-4 week interval)
> - **Subset (N = 200):** Also complete NEO-PI-R, VIA-IS for convergent validity
>
> **Measures:**
>
> **Primary:**
...
>
> **8. AI vs. Anchor Comparison:**
> - **Reliability:** t-test comparing alpha/omega in AI vs. static conditions
> - **Factor structure:** Multi-group CFA testing equivalence across conditions
> - **DIF:** Are AI items more/less biased than anchors?
> - **User experience:** t-test comparing completion time, satisfaction ratings
> - **Decision rule:** AI acceptable if no significant degradation (p > .05) or small effect sizes (d < .20)
>
...
> **Decision Gates:**
>
> **Proceed to Phase 3 (Validation Studies) only if:**
...
> 4. ✅ **AI Performance:** No significant degradation vs. anchors (reliability, validity, bias)
...
**Funding needs (3-year total: $225K-375K):**

**Phase 1 (Year 1): $100K-150K**
- Expert consultation ($25K-50K)
- Pilot study ($75K-100K)
- Development completion ($20K-30K)

**Phase 2 (Year 2, contingent on pilot success): $75K-125K**
- Validation studies ($50K-75K)
- Public beta launch ($25K-50K)

**Phase 3 (Year 3+, contingent on validation): $50K-100K**
- Translation/cultural adaptation ($30K-75K per language)
- Expanded validation research
...
**Timeline for decision:** Ideally by end of Q1 2025 to align with expert consultation phase.
...
**Why this might interest you:**
- **Novel research question:** Can LLM-generated assessment items maintain psychometric equivalence to validated anchors?
...
1. **IRT parameter estimation:** Minimum sample size for stable GRM parameters across 36 facets? Can we use IPIP anchor calibrations or need fresh data?
2. **AI item validation:** Can AI items inherit anchor IRT parameters based on NLI alignment (entailment > 0.85), or must we empirically recalibrate each item?
...
- Access to novel dataset (N = 1,000+ diverse participants, AI vs. anchor comparison)
- Research grants acknowledgment for contributions
- Platform for testing methodological innovations (e.g., Bayesian IRT with LLM priors)
...
- **Open source:** Contribute to client libraries, example apps, documentation when we publish code (Q1 2025)
...
**Not ready yet, but coming:** GitHub repository, API documentation site, developer portal. Watch for announcements in 2025.
...
- Email: [To be added]
- Website: [To be added]
- GitHub: [Repository coming Q1 2025]
- Updates: [Newsletter/blog coming Q1 2025]
